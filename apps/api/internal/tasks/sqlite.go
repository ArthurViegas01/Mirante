...
const taskCols = `id, titulo, status, prioridade, prazo, project_id, job_id, created_at, updated_at`

func scanTask(s rowScanner) (*Task, error) {
	var (
		t                       Task
		idStr, status, prio     string
		prazo, projectID, jobID sql.NullString
		createdAt, updatedAt    string
	)
	if err := s.Scan(&idStr, &t.Titulo, &status, &prio, &prazo, &projectID, &jobID, &createdAt, &updatedAt); err != nil {
...
func (r *sqliteRepo) Create(ctx context.Context, t *Task) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO tasks (id, titulo, status, prioridade, prazo, project_id, job_id)
		 VALUES (?, ?, ?, ?, ?, ?, ?)`,
		string(t.ID), t.Titulo, string(t.Status), string(t.Prioridade),
		nullableStr(t.Prazo), nullableStr(t.ProjectID), nullableStr(t.JobID))
	return err
}
...
func (r *sqliteRepo) List(ctx context.Context, f ListFilter) ([]*Task, error) {
	...
	if f.Prioridade != "" {
		conds = append(conds, "prioridade = ?")
		args = append(args, f.Prioridade)
	}
	if len(conds) > 0 {
		query += " WHERE " + strings.Join(conds, " AND ")
	}
	// Nearest deadlines first; undated tasks last; newest as the tiebreak.
	query += ` ORDER BY prazo IS NULL, prazo, created_at DESC`
	...
	// Load tags per task (single user; few tasks).
	for _, t := range out {
...
func (r *sqliteRepo) Update(ctx context.Context, t *Task) error {
	res, err := r.db.ExecContext(ctx,
		`UPDATE tasks SET titulo = ?, status = ?, prioridade = ?, prazo = ?,
		 project_id = ?, job_id = ?, updated_at = ? WHERE id = ?`,
		t.Titulo, string(t.Status), string(t.Prioridade), nullableStr(t.Prazo),
		nullableStr(t.ProjectID), nullableStr(t.JobID), idb.FormatTime(time.Now()), string(t.ID))
	if err != nil {
		return err
	}
...
// nullableStr maps an empty/absent optional string to a SQL NULL.
